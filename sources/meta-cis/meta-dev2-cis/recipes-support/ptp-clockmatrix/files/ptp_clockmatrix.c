...
static inline int idtcm_read(struct idtcm *idtcm,
			     ...
			     u16 count)
{
	int err = regmap_bulk_read(idtcm->regmap, module + regaddr, buf, count);
	int i = 0;
	while (i < count)
	{
		//dev_warn(idtcm->dev, "read addr[0x%x] index [i] = %d", __func__, regaddr, *(buf + i));
		i++;
	}
	return err;
}
...
static int wait_for_boot_status_ready(struct idtcm *idtcm)
{
	...
	do {
		...
		if (err)
			return err;
		dev_warn(idtcm->dev, "%s status: %d", __func__, status);
		if (status == 0xA0)
			return 0;
