...
_Bool ESP8266_WaitRecive(void)
{
	...
	esp8266_cntPre = esp8266_cnt;					//置为不同
...
_Bool ESP8266_SendCmd(char *cmd, char *res)
{
	...
	while(timeOut--)
	{
		if(ESP8266_WaitRecive() == REV_OK)							//如果收到数据
		{
			...
			if(strstr(snap, res) != NULL)							//如果检索到关键词
			{
				ESP8266_Clear();									//清空缓存
				return 0;
			}
...
unsigned char *ESP8266_GetIPD(unsigned short timeOut)
{
	...
	do
	{
		...
		{
			ptrIPD = strstr((char *)(void *)esp8266_buf, "IPD,");		//搜索"IPD"头
			if(ptrIPD == NULL)											//如果没找到，可能是IPD头延时，我们只是需要等待一会，但不能长时间等待
			{
